...
import { fetchGetinstitution } from '@/service/api';
...
export function useTable<A extends NaiveUI.TableApiFn>(config: NaiveUI.NaiveTableConfig<A>) {
   ...
   */
  async function getDataByPage(pageNum: number = 0) {
    updatePagination({
      skip: pageNum,
      limit: pagination.limit
    });
  ...
  async function fromParams(data:object) {
...
export function useTableOperate<T extends TableData = TableData>(data: Ref<T[]>, getData: () => Promise<void>) {
  ...
  function handleEdit(id: T['objectId']) {
    operateType.value = 'edit';
    const findItem = data.value.find(item => item.objectId === id) || null;
